...
Original file is located at
    https://colab.research.google.com/drive/1scSNJ6d4TIBNrQ9k9ezGPmgoDp79YdJW
"""
...
Решать будем явным методом Рунге-Куты 4 порядка
Отдельно стоит отметить, что, во-первых, систему ДУ не является жесткой, поэтому использование явного метода целесообразно.
Во-вторых, ожидаемые решения принадлежат области устойчивости ЯМРК(4)
$$ЯМРК(4):$$
...
**Важно, график в 3D необходимо покрутить, в том виде, в каком он сейчас, он плохо отражает действительность. В Google Colab с этим могут быть проблемы, лучше запускать локально или с локальной YupiterNotebook**
"""
